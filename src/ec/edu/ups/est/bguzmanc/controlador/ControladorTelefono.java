	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(opcion.equals("crear_telefono")) {
			...
			if(validacionTelefono=="") {
				...
				tel = new Telefono(telefono11, tipo, operadora);
		...
		if(opcion.equals("modificar_telefono")) {
			...
			if(actualizar != null && ban == true) {
				...
				if(nuevoNumero.equals(ttt)) {
					telFInal = new Telefono(nuevoNumero, nuevoTipo, "");
					telefonoDAO.update(telFInal);
					request.setAttribute("valOk", "Actualizado el numero " + telFInal.getNumero());
				}else {
					telFInal = new Telefono(nuevoNumero, nuevoTipo, "");
					telefonoDAO.updateNumero(telFInal, ttt);
			...
			Set<Telefono> telefonos = telefonoDAO.findByUsuario(us.getCedula());
